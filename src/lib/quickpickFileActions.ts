...
export function setupfzfMenuActions() {
  reset();
  updateAppState('IDLE');
  ...
  function next() {
    cx.fzfMenuActionsSelected = (cx.qp.selectedItems as QPItemRgMenuAction[]).map((item) => item.data.rgOption);

    // if no actions selected, then use the current query as a custom command to rg
    if (!cx.fzfMenuActionsSelected.length && cx.qp.value) {
      cx.fzfMenuActionsSelected.push(cx.qp.value);
      cx.qp.value = '';
    }
